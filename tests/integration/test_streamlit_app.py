...
def test_judge_comparison_runs_without_credentials_and_exposes_evidence() -> None:
    """One click produces both traces and visibly reports safe provider fallback."""
    app = AppTest.from_file(APP, default_timeout=APP_TIMEOUT_SECONDS).run()
    app.button[0].click().run()

    assert not app.exception
    assert len(app.success) == 1
    assert len(app.metric) == JUDGE_METRIC_COUNT
    assert len(app.tabs) == TAB_COUNT
    assert len(app.dataframe) >= MINIMUM_DATAFRAME_COUNT + 1
    markdown = "\n".join(item.value for item in app.markdown)
    captions = "\n".join(item.value for item in app.caption)
    assert "Judge comparison / context versus control" in markdown
    assert "Safe fallback path" in markdown
    assert "No behavior-level difference was observed" in captions
